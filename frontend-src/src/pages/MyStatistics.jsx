...
const MyStatistics = () => {
  const { t } = useTranslation('my_statistics');
  const { user } = useAuth();
    ...
    fetch(`/api/my_statistics.php?orcid=${encodeURIComponent(user.orcid)}&range=${timeRange}`)
      ...
      .then(resp => {
        if (resp.status === 'success') {
          ...
          setError(resp.message || t('load_failed'));
        }
      })
      .catch(err => setError(`${t('error_prefix')} ${err.message}`))
      .finally(() => setLoading(false));
  }, [user?.orcid, timeRange, t]);
  ...
  const cards = [
    { key: 'views',     value: (data?.summary?.views      || 0).toLocaleString(),  growth: data?.summary?.viewsGrowth     || '+0%', icon: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z', color: 'bg-indigo-50 text-indigo-600' },
    { key: 'citations', value: (data?.summary?.citations  || 0).toLocaleString(),  growth: data?.summary?.citationsGrowth || '+0%', icon: 'M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z', color: 'bg-purple-50 text-purple-600' },
    { key: 'hindex',    value:  data?.summary?.hIndex     || '0',                  growth: data?.summary?.hIndexGrowth    || '+0',  icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6', color: 'bg-amber-50 text-amber-600' },
    { key: 'impact',    value: (data?.summary?.avgImpactScore || 0).toFixed(1),    growth: data?.summary?.impactGrowth    || '+0%', icon: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z', color: 'bg-green-50 text-green-600' },
  ];
  ...
  return (
    <main className="pt-28 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            <option value="1">{t('range.1')}</option>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {cards.map((stat) => (
          <div key={stat.key} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm mb-8">
        ...
        <ResponsiveContainer width="100%" height={350}>
          <AreaChart data={impactTrends}>
            ...
            <Area type="monotone" dataKey="views" name={t('chart.views')} stroke="#6366f1" strokeWidth={3} fillOpacity={1} fill="url(#colorViews)" />
            <Line type="monotone" dataKey="citations" name={t('chart.citations')} stroke="#a855f7" strokeWidth={3} dot={{ r: 4 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="text-lg font-bold text-gray-900 mb-4">{t('sections.sdg_profile')}</h3>
          <div className="flex items-center justify-center">
            <ResponsiveContainer width="100%" height={300}>
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={sdgData}>
                ...
                <Radar name={t('chart.contribution')} dataKey="A" stroke="#6366f1" fill="#6366f1" fillOpacity={0.4} strokeWidth={2} />
                <Legend />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="text-lg font-bold text-gray-900 mb-4">{t('sections.top_countries')}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={demographics} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="views" name={t('chart.views')} fill="#10b981" radius={[0, 4, 4, 0]} barSize={20} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Citation Sources */}
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="text-lg font-bold text-gray-900 mb-4">{t('sections.citation_sources')}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={sources} margin={{ top: 5, right: 20, left: 20, bottom: 25 }}>
              ...
              <YAxis stroke="#6b7280" fontSize={12} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="count" name={t('chart.citation')} fill="#f59e0b" radius={[4, 4, 0, 0]} barSize={30} />
            </BarChart>
          </ResponsiveContainer>
        </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium text-center">{t('table.views')}</th>
                  <th className="px-4 py-3 font-medium text-center">{t('table.citations')}</th>
                  <th className="px-4 py-3 font-medium text-center rounded-tr-lg">{t('table.score')}</th>
                </tr>
              </thead>
