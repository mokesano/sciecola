  ...
  return (
    <li className="flex items-start gap-4 border-b border-slate-100 py-4 first:pt-0 last:border-0 last:pb-0">
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        ...
        {item.trend && (
          <span className={`text-[15px] font-semibold tabular-nums ${trendClass}`}>{item.trend}</span>
        )}
        {start && end && (
          <span className="font-mono text-xs tabular-nums text-slate-400">{start}–{end}</span>
        )}
      </div>
    </li>
  );
  ...
  return (
    <section className="flex h-full flex-col rounded-lg border border-slate-200 bg-white">
      <header className="flex items-center justify-between border-b border-slate-200 px-5 py-3.5">
        <h3 className="text-[15px] font-semibold uppercase tracking-[0.12em] text-slate-900">
          {t('insights_ai.title')}
        </h3>
        <Link to="/insights"
          className="text-[15px] text-slate-500 underline-offset-4 hover:text-indigo-700 hover:underline">
      ...
      <div className="flex-grow px-5">
        {insights.length === 0 ? (
          <p className="py-10 text-center text-[15px] text-slate-500">{t('insights_ai.empty')}</p>
        ) : (
          <ul>
      ...
      <div className="border-t border-slate-200 px-5 py-3">
        <Link to="/insights"
          className="flex items-center justify-center gap-2 rounded border border-slate-300 py-2 text-[15px] font-medium text-slate-700 transition-colors hover:border-slate-400 hover:bg-slate-50">
          {t('insights_ai.cta')}
